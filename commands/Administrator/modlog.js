...
module.exports = {
        ...
        usage: "modlog <channel mention | channel ID | channel name>",
    ...
    if (!args[0]) {
      ...
      if (message.guild.channels.cache.has(b)) {
        return message.channel.send({
          embed : {
            color : `GREEN`,
            description : `Mod-log Channel Set In This Server Is \`${channelName.name}\`!`
          }
        }
        );
      } else
        ...
        if (!channel || channel.type !== 'text') return message.channel.send({
          embed : {
            description : `❌Please Enter A Valid Text Channel!`
          }
        });
        ...
        try {
            ...
            if (channel.id === a) {
                return message.channel.send({
                  embed : {
                    description : `❌This Channel is Already Set As Mod-log Channel!`
                  }
                })
            } else {
                bot.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send({
                  embed : {
                    color: `GREEN`,
                    description : `Mod-log Channel Set!`
                  }
                })
                ...
                message.channel.send({
                  embed : {
                    color :`GREEN`,
                    description : `✅Mod-log Channel Has Been Set Successfully in \`${channel.name}\`!`
                  }
                })
            }
        } catch {
